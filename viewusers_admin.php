...
<header>
        <nav>
            <ul class="justify-content-center nav nav-pills">
                ...
                <li class="nav-item">
                    <a href="#" class="nav-link active">Ver Usuarios</a>
                </li>
                <li class="nav-item">
    ...
    <div class="container">
        <h1 class="text-center m-3">Tabla de Usuarios</h1>
    <?php
    $oUsers = new usersDb(DB_HOST, DB_USER, DB_PASS, DB_NAME);
    $result = $oUsers->allusers();
    echo "<table class='table table-striped border'><tr><th>ID</th><th>Usuario</th><th>Telefono</th></tr>";
    while ($row = $result->fetch_assoc()) {
        if ($row['mail'] != "admin@admin.com") {
            echo "<tr><td>'" . $row['idusers'] . "'</td><td> '" . $row['mail'] . "' </td><td>'" . $row['phone'] . "'</td></tr>";
        }
    }
    echo "</table>";
